...
public class TeacherDao extends CommonDao {
    ...
    public ArrayList<Teacher> getByType(Integer type) {
        ...
        try {
            Connection conn = getConnection();
            PreparedStatement ps = conn.prepareStatement("SELECT * FROM teacher WHERE teacher_type = ? ORDER BY teacher_type ASC, first DESC, second DESC, teacher_id DESC, name ASC");
            ps.setInt(1, type);
            ResultSet rs = ps.executeQuery();
